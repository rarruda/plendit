class SmsService
  ...
  def send_sms
    if not valid_to?
      LOG.error "sms_to is not a valid norwegian mobile number. Refusing to send sms.", sms_to: @sms_to
    else
      LOG.info "SMS: From: #{from} To: #{@sms_to} Body: '#{@sms_body}'", sms_to: @sms_to
