class PriceItem < ActiveRecord::Base
  ...
  def amount_should_be_reasonable
    prev_price_item = PriceItem.where( "price_id = ? AND unit = ? AND effective_from_unit < ?", self.price_id, PriceItem.units[self.unit.to_sym], self.effective_from_unit ).order(effective_from_unit: :desc).limit(1).first
    return true if prev_price_item.nil?
