...
class Ad < ActiveRecord::Base
  ...
  settings index: { number_of_shards: 4 } do
    mappings do
      ...
      indexes :tags,          type: :string, analyzer: 'norwegian'
  ...
  def boats_with_license_must_have_terms_acceptance_or_reg_num
    if self.boat? && self.boat_license_required && self.registration_number.blank? && !self.accepted_boat_insurance_terms
  ...
  def build_payin_rule
    self.payin_rules.build(unit: 'day', effective_from: 1)
  end
